...
import numpy as np # Import numpy for the confusion matrix labeling

# 1. Load and preprocess data
df = pd.read_csv(r"C:\Users\Jashmina\Downloads\Telco-Customer-Churn.csv")

# Define the 9 key features as per your updated methodology, including 'Churn'
...
# Select only these 9 features from the DataFrame and create a copy to avoid warnings
df = df[selected_original_features].copy()

# Drop 'customerID' if it exists (it won't if you only selected 9 features above, but as a safeguard)
if 'customerID' in df.columns:
    df.drop('customerID', axis=1, inplace=True)

# Handle 'TotalCharges' as per updated methodology: it's dropped because it's not in the 9 selected features
...
# Identify categorical columns for one-hot encoding *after* feature selection, excluding 'Churn'
...
# This shows how your categorical features have been converted into numerical (0/1) columns.
...
scaler = MinMaxScaler()
# Ensure only 'tenure' and 'MonthlyCharges' are scaled, as per your reduced feature set
...
# Create a pipeline that first applies SMOTE and then trains the Logistic Regression model
pipe_lr = Pipeline([('smote', smote), ('model', lr_model)])

# Use GridSearchCV to find the best hyperparameters for the pipeline
...
def plot_confusion_matrix(y_true, y_pred, title, filename):
    ...
    # Create the annotation array with counts and labels
    group_names = ['TN','FP','FN','TP']
    group_counts = ["{0:0.0f}".format(value) for value in cm.flatten()]
    
    # Combine counts and labels for annotation
...
# 4. Print classification reports for both baseline and SMOTE-enhanced LR
...
# 5. Plot confusion matrices for both baseline and SMOTE-enhanced LR
...
# 7. Plot ROC curves for Logistic Regression Baseline vs. SMOTE
...
# 8. Plot Feature Coefficients for Logistic Regression SMOTE Model (for interpretability)
